...
(/.) :: (HasValue (V b1) i, HasValue (V b2) i, Fractional c) => V b1 r c -> V b2 r c -> V (TyComp i TyVal) r c
(/.) a b = liftOp2 (/) a b 

(&&.), (||.) :: (HasValue (V b1) i , HasValue (V b2) i) => V b1 r Bool -> V b2 r Bool -> V (TyComp i TyVal) r Bool
(&&.) a b = liftOp2 (&&) a b
(||.) a b = liftOp2 (||) a b

(~.) :: (HasValue (V b) i) => V b r Bool -> V (TyComp i TyVal) r Bool
(~.) a = C $ do 
  a' <- val a
  return $ Lit $ not a'
